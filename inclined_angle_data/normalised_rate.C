...
void normalised_rate() {
    ...
    for (int j = 0; j < fit.size(); ++j) {
        ...
        for (int i = 0; i < fit[0].size(); ++i) {
            ...
            gr[j]->SetPoint(i, x[i], fit[j][i]->GetParameter(1)/fit[0][i]->GetParameter(1));
            gr[j]->SetPointError(i, 0, propagate_error_division(fit[j][i]->GetParameter(1), fit[j][i]->GetParError(1),fit[0][i]->GetParameter(1), fit[0][i]->GetParError(1) ));
    ...
    for (int i = 0; i < gr.size(); ++i) {
        if (i == 0) {
            ...
            gr[i]->SetMinimum(0.9); // Adjust buffer
            gr[i]->SetMaximum(1.02); // Adjust buffer
            gr[i]->Draw("ALP");
            gr[i]->SetTitle(";;Normalised Rate");
        } else {
